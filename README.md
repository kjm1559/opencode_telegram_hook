...
Extracts summary body text from OpenCode's summary agent response. Preserves body text across subsequent `message.updated`/`session.updated` events.
...
3. **Summary generated** → `message.updated` captures metadata, `message.part.updated` captures summary text
